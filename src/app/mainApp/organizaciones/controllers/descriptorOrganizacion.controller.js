...
(function () {
  ...
  /* @ngInject */
  function descriptorOrganizacionController($translate, $scope, Descriptor, moment, Restangular, Translate, toastr, $mdDialog) {
    var vm = this;
    vm.language = "en";
    ...
    function activate() {
      ...
      Restangular.all('Organizacion').customGET().then(function (res) {
        vm.organizaciones = res.Organizacion;
        var proms = Descriptor.getTipoDescriptorByClasificacion('Organizacion');
    ...
    function showDescriptor() {
      if (vm.descriptor.idTipoDescriptor != undefined && vm.descriptor.idTipoDescriptor != null) {
        var promise = Descriptor.callAssosciated(vm.descriptor.idTipoDescriptor);
    ...
    function selectedItemChange() {
      if (vm.selectedItem.id != undefined && vm.selectedItem != null) {
        Restangular.all('Organizacion').one('Descriptor', vm.selectedItem.id).customGET().then(function (res) {
          ...
          console.log(vm.descriptorPersonas);
        }).catch(function (err) {
    ...
    $scope.addItem = function () {
      ...
      if (vm.descriptor.id == null) {
        ...
        Restangular.all('Organizacion').one('Descriptor', vm.descriptor.pivot.id).customPUT(vm.descriptor).then(function (res) {
          toastr.success(vm.successText, vm.successStoreText);
          //Limpiamos las variables ligadas a formulario
          vm.descriptor.idDescriptor = null;
          vm.descriptor.FechaInicio = null;
    ...
    function edit(item) {
      if (item != undefined) {
        vm.descriptor = angular.copy(item);
        vm.descriptor.idTipoDescriptor = item.idTipoDescriptor;
        vm.descriptor.idDescriptor = item.pivot.idDescriptor;
        vm.descriptor.TipoResultado = item.pivot.TipoResultado;
        vm.descriptor.NumeroRegistro = item.pivot.NumeroRegistro;
        vm.descriptor.FechaInicio = item.pivot.FechaInicio;
        vm.descriptor.FechaTermino = item.pivot.FechaTermino;
        var promise = Descriptor.callAssosciated(vm.descriptor.idTipoDescriptor);
        promise.then(function (res) {
          vm.descriptores = res.Descriptor;
        });
